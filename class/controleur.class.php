	...
	public function retourne_formulaire_enfant($type,$idenfant) 
	{
		...
		<article >
			...
			<form id="formenfant" method="post" >
				...
				<input type="hidden" name="id_enfant" value='.$id_enfant.'> 
		...
		<article>
			...
			$("#formenfant").submit(function( e ){
				...
				if($("#submit").prop("value")=="Modifier")
					{$url="ajax/valide_modif_enfant.php";}

				if($("#submit").prop("value")=="Supprimer")
					{$url="ajax/valide_supp_enfant.php";}
				...
				<table>
					...
					if ($result != false) {
						...
						// parcourir chaque ligne sélectionnée
						{
							...
							<td Align=center>
							<input onClick="this.form.submit();" type="checkbox" name="nom_checkbox[]" value="' . $row->id_enfant . '" /></td>
						</tr>';
						}
					}
					$retour = $retour . '
